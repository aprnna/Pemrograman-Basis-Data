...
export default function MenuCards() {
  ...
  async function getMenu() {
    setLoading(true);
    const { data } = await fetchApi("/menu?tersedia=true", "GET");
  ...
  return (
    <div
      className={`flex flex-wrap ${cart.length > 0 ? "justify-around" : "justify-between"} px-12 gap-8 pb-6 h-max ${loading ? "w-full" : "w-auto"}`}
    >
      {loading ? (
          ...
          {menu.map((item: any) => (
            <div key={item.id} className="flex drop-shadow-md">
              ...
              >
                <img
                  alt={item.nama}
                  className={`border w-full h-auto ${cart.length > 0 ? "max-h-[250px] min-h-[250px]" : "max-h-[270px] min-h-[270px]"} rounded-xl`}
                  src={item.foto}
                />
